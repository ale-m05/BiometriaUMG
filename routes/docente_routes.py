...
def register_docente_routes(app):
    ...
    def dashboard_docente():
        ...
        cursor.close()
        conn.close()
        return render_template('catedratico/mis_cursos.html', cursos=cursos, usuario=usuario, fecha_hoy=date.today())
    ...
    def ver_asistencia_curso(id_curso):
        ...
        if 'id_rol' in rp_cols:
            ...
            cursor.execute(f"""
                ...
                LEFT JOIN salones sal ON ac.id_salon = sal.id_salon
                LEFT JOIN jornadas j ON ac.id_jornada = j.id_jornada
                LEFT JOIN horarios h ON ac.id_asignacion = h.id_asignacion
                WHERE c.id_curso = %s
                  AND r.id_persona = %s
                  AND ro.nombre = 'catedratico'
                  {activo_clause}
        ...
        cursor.execute("""
            SELECT p.id_persona, p.nombre, p.apellido, COALESCE(p.correo_institucional, p.correo_personal) AS correo, p.foto, p.id_persona AS carnet
        ...
        try:
            ...
            cursor.execute("""
                SELECT id_persona,
                       nombre,
                       apellido,
                       COALESCE(correo_institucional, correo_personal) AS correo
                FROM personas
                WHERE id_persona = %s
            """, (usuario['id_persona'],))
            ...
            cursor.execute("""
                SELECT p.id_persona,
                       p.nombre,
                       p.apellido,
                       COALESCE(p.correo_institucional, p.correo_personal) AS correo,
                       p.id_persona AS carnet
                FROM inscripciones i
                JOIN roles_persona r ON i.id_rol_persona = r.id_rol_persona
                JOIN personas p ON r.id_persona = p.id_persona
                JOIN asignacion_cursos ac ON i.id_asignacion = ac.id_asignacion
            ...
            ruta_pdf, nombre_archivo = generar_pdf_asistencia(
                curso,
                docente,
                estudiantes_pdf,
                fecha_hoy
